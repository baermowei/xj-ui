<template>
  <li
    class="xj-menu__submenu"
    :class="[
      this.active ? 'xj-menu__submenu--active' : '',
      this.isOpen ? 'xj-menu__submenu--opened' : '',
      this.disabled ? 'xj-menu__submenu--disabled' : ''
    ]"
    ...
    <transition name="slide-up" v-else>
      <div
        class="xj-dropdown__popover"
        placement="placementValue"
        :style="dropStyle"
        ref="popover"
        v-show="isOpen">
        <ul class="xj-dropdown-menu">
          <slot></slot>
        </ul>
      </div>
    </transition>
  ...
  export default {
    ...
    data () {
      return {
        ...
        parentMenu: findComponentUpward(this, 'Menu')
      }
    },
    computed: {
      mode () {
